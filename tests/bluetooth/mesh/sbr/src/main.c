...
static void check_fill_all_bt_entries3(void)
{
	int err;
	for (int i = 0; i < TEST_VECT_SZ; i++) {
		err = sbr_bt_add3(test_vector[i].direction, test_vector[i].netidx1,
			test_vector[i].netidx2, test_vector[i].addr1, test_vector[i].addr2);
		if (i != CONFIG_BT_MESH_SBC_BRIDGING_TABLE_ITEMS_MAX) {
			zassert_equal(err, 0);
		} else {
...
static void check_delete_all_bt_entries3(void)
{
	int err;
	for (int i = TEST_VECT_SZ; i >= 0; --i) {
		err = sbr_bt_remove3(test_vector[i].netidx1, test_vector[i].netidx2,
			test_vector[i].addr1, test_vector[i].addr2);
		zassert_equal(err, 0);
	}
}
...
ZTEST(bt_mesh_sbr_bt, test_reset_normal3)
{
	sbr_bt_init3();

	/* Test add entries to bridging table. */
	check_fill_all_bt_entries3();

	/* Test remove entries from bridging table, and then fill it again. */
	check_delete_all_bt_entries3();
	check_fill_all_bt_entries3();
