...
static bool msg_cache_match(struct net_buf_simple *pdu, const struct bt_mesh_net_rx *rx, uint8_t nid)
{
	uint16_t i;
			...
			const struct bt_mesh_net_cred *cred)
{
	...
	if (rx->net_if == BT_MESH_NET_IF_ADV && msg_cache_match(out, rx, cred->nid)) {
