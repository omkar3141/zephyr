...
struct app_key {
	uint16_t net_idx; /* NetKey Index the AppKey is bound to. */
	uint16_t app_idx; /* Application Key Index. */
	bool updated;     /* True if the AppKey has a second key for KR. */
	struct bt_mesh_app_cred {
		uint8_t id;             /* Application ID. */
		struct bt_mesh_key val; /* Application Key value. */
	} keys[2];
};
